...
class House:
    def __init__(self, initial_temperature, outside_temperature, max_temperature, times):
        ...
        heater5=Heater(room4, [93,112,131,150], max_temperature)
    ...
    def create_animation(self):
        ...
        house_matrix = self.create_house_matrix(0)
        im = ax.imshow(house_matrix, cmap="plasma", animated=True)
        plt.colorbar(im, label="Temperatura (K)") 

        ax.set_title("T")
        time_text = ax.text(0.05, 0.95, '', transform=ax.transAxes, fontsize=12,color='white')

        def update(t):
            house_matrix = self.create_house_matrix(t)
            im.set_array(house_matrix)
            time_in_hours = t * ht / 3600  # Przeliczenie na godziny
            time_text.set_text(f"Time: {time_in_hours:.1f} h")
            return im, time_text

        anim = FuncAnimation(fig, update, frames=range(0, self.times, 60), blit=False)
        plt.show()


    def main(self, czas_wyjścia_z_domu, czas_wejścia_do_domu, stare_max, nowe_max):
        ...
        for t in np.arange(1, self.times):  
            ...
            for heater in self.heaters:
                if t == czas_wyjścia_z_domu:
                    heater.max_temperature = nowe_max
                if t == czas_wejścia_do_domu:
                    heater.max_temperature = stare_max
            ...
            if t % (7200) == 0:
                ...
                plt.imshow(house_matrix, cmap="plasma", vmin=269.15, vmax=303.15)  # Fixed range for temperature
                plt.colorbar(label='Temperature (K)')
                plt.title(f"Temperatura o godzinie {t * ht / 3600:.1f} h")  # Fixed title formatting
                plt.savefig(f"Temperatura_grzejniki_{t}.png")  # Unique file names
                plt.close()
